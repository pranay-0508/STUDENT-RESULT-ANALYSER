...
public class STUDENTRESULTANALYSER {
        ...
        if (file.exists()) {
            try {
                ...
                for (int i = 0; i < studentCount; i++) {
                    ...
                    for (int s = 0; s < subjectCount; s++) m[s] = Double.parseDouble(fr.nextLine());
                    ids.add(id);
                    names.add(name);
                    marks.add(m);
                }
    ...
    Integer[] order2 = new Integer[ids.size()];
    double[] percentages2 = new double[ids.size()];
    for (int i = 0; i < ids.size(); i++) {
        order2[i] = i;
        double sum = 0;
        for (double m : marks.get(i)) sum += m;
        percentages2[i] = (sum / (subjectCount * MAX_MARKS_PER_SUBJECT)) * 100;
    }
    Arrays.sort(order2, (a, b) -> Double.compare(percentages2[b], percentages2[a]));
    StringBuilder top = new StringBuilder("┌───────────────┬──────────────────────┬");
    for (int s = 0; s < subjectCount; s++) top.append("────────┬");
    top.append("────────┬────────┬──────────┬────────┬────────┐");
    StringBuilder mid = new StringBuilder("├───────────────┼──────────────────────┼");
    for (int s = 0; s < subjectCount; s++) mid.append("────────┼");
    mid.append("────────┼────────┼──────────┼────────┼────────┤");
    StringBuilder bottom = new StringBuilder("└───────────────┴──────────────────────┴");
    for (int s = 0; s < subjectCount; s++) bottom.append("────────┴");
    bottom.append("────────┴────────┴──────────┴────────┴────────┘");
    System.out.println(top);
    System.out.printf("│ %-13s │ %-20s │", "ID", "Name");
    for (int s = 1; s <= subjectCount; s++) System.out.printf(" Sub%-3d │", s);
    System.out.printf(" %-6s │ %-6s │ %-8s │ %-6s │ %-6s │\n",
            "Total", "Avg", "Percent", "Result", "Grade");
    System.out.println(mid);
    for (int idx : order2) {
        double sum = 0;
        boolean pass = true;
        System.out.printf("│ %-13d │ %-20s │", ids.get(idx), names.get(idx));
        for (int s = 0; s < subjectCount; s++) {
        ...
        String grade = pass ?
                (percent >= 85 ? "A" :
                 ...
                 percent >= 40 ? "D" : "F")
                : "F";
        System.out.printf(" %-6.2f │ %-6.2f │ %-8.2f │ %-6s │ %-6s │\n",
    ...
    public static boolean idExists(ArrayList<Long> ids, long id) {
